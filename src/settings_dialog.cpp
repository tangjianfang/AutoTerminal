...
constexpr int kHintH           = 16;   // one hint text line (xs); hints get 2
...
protected:
    ...
    bool on_command(int id, HWND /*src*/, UINT code) override {
        switch (id) {
            ...
            case IDC_HK_PREVIEW_CAPTURE:  if (code == BN_CLICKED) { start_capture(CapPreview);  return true; } break;
...
private:
    ...
    // are text_secondary at sm). Word-wraps onto a second line — several
    // hint texts exceed one field width.
    void add_hint(int x, int y, int w, std::wstring_view text, int id) {
        nfui::ControlCreateParams p{inst_, hwnd(), id, text, x, y, w,
                                    px(kHintH * 2)};
        auto lbl = std::make_unique<nfui::StaticText>();
        (void)lbl->inject_theme(&palette_, &fonts_);
        nfui::TextStyle ts{};
        ...
        ts.align_v      = nfui::StaticTextAlignV::top;
        ts.single_line  = false;   // wrap instead of ellipsize
        (void)lbl->set_style(ts);
        (void)lbl->create(p);
        controls_.push_back(std::move(lbl));
    }
    ...
    nfui::Button& button_for(int id) {
        switch (id) {
            ...
            case IDC_HK_PREVIEW_CAPTURE:  return btn_preview_cap_;
    ...
    void refresh_hotkey_labels() {
        ...
        std::wstring v = format_hotkey(preview_hk());
        ...
        if (cap_ == CapPreview)  v = L"Press a key combo (Esc to cancel)...";
        ...
        SetWindowTextW(GetDlgItem(hwnd(), IDC_HK_PREVIEW_DISPLAY), v.c_str());
    }
    ...
    void finish_capture(UINT vk) {
        ...
        if (cap_ == CapPreview)  pending_preview_   = hk;
        cap_ = CapNone;
        refresh_hotkey_labels();
        ...
        if (finished == CapPreview)  focus_id = IDC_HK_PREVIEW_CAPTURE;
    ...
    void on_apply() {
        ...
        if (pending_preview_)   cfg_.hotkey_preview           = *pending_preview_;
        pending_tile_.reset();
        pending_pause_.reset();
        pending_tile_spec_.reset();
        pending_preview_.reset();
    ...
    void on_import_config() {
        ...
        // Discard any pending edits — the imported config replaces the live
        // values, so any in-flight capture (incl. an in-progress cap_ state)
        // must also be cleared via cancel_capture().
        cancel_capture();
        populate_monitors();
        populate_log_levels();
        apply_text_widgets();
    ...
    Hotkey preview_hk()   const { return pending_preview_   ? *pending_preview_   : cfg_.hotkey_preview; }
    ...
    std::optional<Hotkey> pending_preview_;
